...
$.extend(wot, { crypto: {
	...
	getnonce: function(seed, witness)
	{
		...
		var data = "nonce:" + wot.version +
						...
						":" + Date.now() +
						":" + Math.random() +
						":" + ((witness || wot.witness || {}).id || "") +
						":" + (seed || "");

		return this.bintohex(this.sha1.sha1str(data));
	},

	decrypt: function(data, nonce, options)
	{
		try {
			if (data && nonce) {
				var key = options && options.witness_key ? 
					options.witness_key : wot.witness.key;
				var index = (options || {}).index;
				var response_str = (options || {}).response_str;
				...
				var hash_string = nonce + index;
				if (response_str) { 
					hash_string = "response-" + hash_string;
				}
				
				// debug - 
				// var atob_result = atob(data);
				// var strtobin = this.strtobin(atob_result);
				// var sha1 = this.sha1.hmacsha1hex(key, nonce);
				// var arc4create = this.arc4.create(sha1);
				// var arc4crypt = this.arc4.crypt(arc4create, strtobin);
				// var result = this.bintostr(arc4crypt);

				if (key) {
					return this.bintostr(
						this.arc4.crypt(
							this.arc4.create(
								this.sha1.hmacsha1hex(
									key,
									hash_string
								)
							),
	...
	encrypt: function(data, nonce, key)
	{
		try {
			if (data && nonce) {
				if (!key) { 
				...
				if (key) {
					// debug - 
					// var sha1 = this.sha1.hmacsha1hex(key, nonce);
					// var strtobin = this.strtobin(data);
					// var arc4create = this.arc4.create(sha1);
					// var arc4crypt = this.arc4.crypt(arc4create, strtobin);
					// var bintostr = this.bintostr(arc4crypt);
					// var result = btoa(bintostr);

					return btoa(
						this.bintostr(
							this.arc4.crypt(
								this.arc4.create(
									this.sha1.hmacsha1hex(key, nonce)
								),
								this.strtobin(data)
							)
						)
