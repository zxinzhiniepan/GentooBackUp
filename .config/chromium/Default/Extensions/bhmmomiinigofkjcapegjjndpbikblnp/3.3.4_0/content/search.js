	...
	onprocess: function(data)
	{
		if (this.matchrule(data.rule, window)) {
			this.processframe(data.rule, window, function(targets) {
				...
				/* load ratings */
				wot.post("search", "get", { rule: data.rule, targets: targets});
			});
		}
		...
		if (window === window.top) {
			window.setInterval(function () {
				if (document.visibilityState !== 'prerender') {
					...
					if (document.location.href !== this.prev_url) {
						this.prev_url = document.location.href;
						wot.post("search", "change", {targets: this.prev_url});
					}
				}
